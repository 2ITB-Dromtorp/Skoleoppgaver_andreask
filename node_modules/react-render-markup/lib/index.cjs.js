...
var reactProps = new Set(['children', 'dangerouslySetInnerHTML', 'key', 'ref', 'defaultValue', 'defaultChecked', 'innerHTML', 'suppressContentEditableWarning', 'suppressHydrationWarning',
// deprecated
'valueLink']);
...
var nodesToElements = function nodesToElements(nodeList, options) {
  ...
  for (var i = 0; i < nodeList.length; i++) {
    ...
    if (node.nodeType === ELEMENT_NODE) {
      ...
      if (options.replace) {
        var replacement = typeof options.replace === 'function' ? options.replace(node) : Object.hasOwn(options.replace, type) ? options.replace[type] : undefined;
...
var Markup = React__namespace.memo(function (_ref) {
  var allowed = _ref.allowed,
    markup = _ref.markup,
    replace = _ref.replace,
    trim = _ref.trim;
  return markup ? nodesToElements(domParse.parse(markup).childNodes, {
    allowed: allowed,
    replace: replace,
    trim: trim
  }) : null;
});
Markup.propTypes /* remove-proptypes */ = process.env.NODE_ENV !== "production" ? {
...
var renderMarkup = function renderMarkup(markup, _temp) {
  ...
  !(allowed == null || Array.isArray(allowed) || typeof allowed === 'function') ? process.env.NODE_ENV !== "production" ? invariant(false, 'Expected `options.allowed` to be an array or function') : invariant(false) : void 0;
  !(replace == null || typeof replace === 'function' || typeof replace === 'object') ? process.env.NODE_ENV !== "production" ? invariant(false, 'Expected `options.replace` to be a function or object') : invariant(false) : void 0;
  !(trim == null || typeof trim === 'boolean') ? process.env.NODE_ENV !== "production" ? invariant(false, 'Expected `options.trim` to be a boolean') : invariant(false) : void 0;
  return markup ? nodesToElements(domParse.parse(markup).childNodes, {
    allowed: allowed,
    replace: replace,
    trim: trim
